...
# Rich output contains Chinese text and emoji. Force UTF-8 on Windows so
# redirected output and legacy GBK consoles do not fail before startup.
...
class AligoCLI:
    ...
    def __init__(self):
        ...
        self._historical_chat_summary = None
        ...
        with self.console.status("初始化中...", spinner="dots"):
            ...
            self.model = OpenAIChatModel(
                ...
                client_kwargs={
                    ...
                    # retry_with_backoff below owns retries and circuit metrics.
                    "max_retries": 0,
                },
                generate_kwargs={
                    "temperature": LLM_CONFIG.get("temperature", 0.7),
                    "max_tokens": LLM_CONFIG.get("max_tokens", 2000),
                    # 智谱扩展参数需要通过 OpenAI SDK 的 extra_body 传递。
                    "extra_body": {
                        "thinking": {
                            "type": LLM_CONFIG.get(
                                "thinking_type",
    ...
    async def process_query(self, user_input: str):
        ...
        intention_started_at = time.time()
        with self.console.status("正在识别意图...", spinner="dots"):
        ...
        intention_elapsed = time.time() - intention_started_at
        ...
        orchestration_started_at = time.time()
        try:
            with self.console.status("正在执行差旅任务...", spinner="dots"):
        ...
        orchestration_elapsed = time.time() - orchestration_started_at
        ...
        self.console.print(
            f"[dim]耗时：意图识别 {intention_elapsed:.1f}s，"
            f"业务执行 {orchestration_elapsed:.1f}s，"
            f"总计 {total_elapsed:.1f}s[/dim]",
        )
    ...
    async def _get_long_term_summary(self, user_input: str = "") -> str:
        ...
        # 2. 可选：使用 LLM 总结其他会话。默认关闭以避免一次额外的
        # 串行模型调用；开启后每个 CLI 会话只生成一次并缓存。
        chat_summary = ""
        if SYSTEM_CONFIG.get("enable_long_term_summary", False):
            if self._historical_chat_summary is None:
                self._historical_chat_summary = (
                    await self.memory_manager.get_long_term_summary_async(
                        max_messages=50,
                    )
                )
            chat_summary = self._historical_chat_summary
        if chat_summary:
...
def main() -> int:
    ...
    if len(sys.argv) > 1 and sys.argv[1].strip().lower() == "health":
        return run_health_check_standalone()
