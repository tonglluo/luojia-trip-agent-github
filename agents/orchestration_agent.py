    ...
    async def _execute_agent(
        ...
        for attempt in range(max_retries + 1):
            ...
            if should_retry:
                ...
                logger.warning(
                    "Agent %s failed (attempt %d/%d), retry in %.1fs: %s",
                    agent_name,
                    attempt + 1,
                    max_retries + 1,
                    delay,
            ...
            error_text = str(error) if error is not None else "未知错误"
            logger.error(
                "Agent execution failed: %s, attempts=%d, error=%s",
                agent_name,
                attempt + 1,
                error_text,
            )
    ...
    def _aggregate_results(
        ...
        for result in results:
            aggregated["results"].append({
                ...
                "retry_count": result["result"].get("retry_count", 0),
                "attempt_count": result["result"].get("attempt_count", 1),
            })
        ...
        aggregated["agent_retry_count"] = sum(
            item["result"].get("retry_count", 0)
            for item in results
        )
