...
class LazyAgentRegistry:
    ...
    def __getitem__(self, agent_name: str):
        ...
        # Keep loader messages ASCII-safe for legacy Windows/GBK consoles.
        self.console.print(
            f"[dim]Loading {agent_name} (from {skill_name})...[/dim]",
        )
        ...
        try:
            ...
            self.console.print(f"[dim]Loaded {agent_name}[/dim]")
            
            return agent_instance
            
        except Exception as e:
            self.console.print(f"[red]Failed to load {agent_name}: {e}[/red]")
            import traceback
            traceback.print_exc()
            raise
