...
class RAGKnowledgeAgent(AgentBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.embedding_model_id = str(model_path_or_id)
        self.embedding_model = SentenceTransformer(model_path_or_id)
        self.embedding_dim = self.embedding_model.get_sentence_embedding_dimension()

        # 初始化 Milvus Lite（本地文件存储）。Milvus Lite 不支持原生
        # Windows，因此连接失败时自动退化为内存余弦检索。
        ...
        if sys.platform.startswith("win"):
            ...
            logger.info(
                "RAG Knowledge Agent initialized with backend=%s",
                self.backend,
            )
            return
        ...
        try:
            ...
            logger.warning(
                "Milvus Lite unavailable (%s). Falling back to in-memory "
                "cosine retrieval.",
                exc,
            )
            self.backend = "in_memory"
        ...
        self.initialized = True
        logger.info("RAG Knowledge Agent initialized with backend=%s", self.backend)

    def _initialize_in_memory_store(self):
        """Index bundled text documents for platforms without Milvus Lite."""
        import hashlib
        import numpy as np

        documents_dir = self.knowledge_base_path.parent / "documents"
        self._fallback_documents = []
        self.embedding_cache_status = "not_available"

        if not documents_dir.exists():
            logger.warning("Knowledge documents directory not found: %s", documents_dir)
            self._fallback_embeddings = None
            return

        for document_path in sorted(documents_dir.glob("*.txt")):
            ...
            for chunk_index, chunk in enumerate(chunks):
                self._fallback_documents.append({
                    "id": len(self._fallback_documents) + 1,
                    "content": chunk,
                    "metadata": {
        ...
        if self._fallback_documents:
            texts = [doc["content"] for doc in self._fallback_documents]
            fingerprint_payload = {
            ...
            cache_path = (
                ...
                / "in_memory_embeddings_cache.npz"
            )
            self._fallback_embeddings = None
            try:
                ...
                if (
                    ...
                    self.embedding_cache_status = "hit"
                    logger.info(
                        "Loaded %d cached local embeddings from %s",
                        len(texts),
                        cache_path,
                    )
            except (OSError, ValueError, KeyError):
                logger.info(
                    "No valid local embedding cache found at %s",
                    cache_path,
                )

            if self._fallback_embeddings is None:
                self._fallback_embeddings = self.embedding_model.encode(
                    texts,
                    normalize_embeddings=True,
                )
                self.embedding_cache_status = "miss_built"
                try:
                    cache_path.parent.mkdir(parents=True, exist_ok=True)
                    np.savez_compressed(
                        cache_path,
                        fingerprint=np.asarray(fingerprint),
                        embeddings=np.asarray(self._fallback_embeddings),
                    )
                    logger.info(
                        "Saved local embedding cache to %s",
                        cache_path,
                    )
                except OSError as exc:
                    logger.warning(
                        "Unable to save local embedding cache: %s",
                        exc,
                    )
        else:
            self._fallback_embeddings = None
        ...
        logger.info(
            "Indexed %d local document chunks for fallback retrieval",
        ...
        try:
            if self.backend == "in_memory":
                ...
                return {
                    ...
                    "total_count": len(self._fallback_documents),
    ...
    def search_knowledge(self, query: str, top_k: Optional[int] = None) -> List[Dict]:
        ...
        try:
            if self.backend == "in_memory":
                ...
                k = min(top_k or self.top_k, len(self._fallback_documents))
                query_embedding = self.embedding_model.encode(
                    query,
                    normalize_embeddings=True,
                )
                scores = np.asarray(self._fallback_embeddings) @ np.asarray(
                    query_embedding,
                )
                best_indices = np.argsort(scores)[::-1][:k]
                return [
                    {
                        **self._fallback_documents[int(index)],
...
"""
        ...
        if generation_error:
            ...
            result["message"] = "知识库检索成功，但答案生成服务暂时不可用"
    ...
    def close(self):
        """关闭 Milvus 连接"""
        if getattr(self, "backend", None) == "milvus_lite" and hasattr(self, 'milvus_client'):
            try:
                if hasattr(self.milvus_client, 'close'):
                    self.milvus_client.close()
